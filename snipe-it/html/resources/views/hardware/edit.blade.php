...
@extends('layouts/edit-form', [
    ...
    'formAction' => ($asset) ? route('hardware.update', ['hardware' => $asset->id]) : route('hardware.store'),
])
...
@section('inputFields')
  ...
  @include ('partials.forms.edit.order_number')
    ...
    if ($asset->id && $asset->location) {
        $currency_type = $asset->location->currency;
    }
    ?>
  ...
  <div class="form-group {{ $errors->has('image_delete') ? 'has-error' : '' }}">
      ...
      <div class="col-md-5">
          ...
          <div style="margin-top: 0.5em">
              <img src="{{ url('/') }}/uploads/assets/{{ $asset->image }}" class="img-responsive"/>
          </div>
      </div>
  </div>
  @endif
